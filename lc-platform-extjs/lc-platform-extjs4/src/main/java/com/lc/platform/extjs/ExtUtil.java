...
public class ExtUtil {
	...
	public static PageBean getPageBean(QueryParams queryParams)
		...
		if (StringUtils.isNotEmpty(dir)) {
			pageBean.addOrder(new Order(sort, OrderType.valueOf(dir
					.toUpperCase())));
		} else if (StringUtils.isNotEmpty(sort)) {
			Sort[] sorts = objectMapper.readValue(sort, Sort[].class);
			for (Sort item : sorts) {
				pageBean.addOrder(new Order(item.getProperty(), OrderType
						.valueOf(item.getDirection().toUpperCase())));
			}
		}
		String filtersJson = queryParams.getFilters();
		if (StringUtils.isNotEmpty(filtersJson)) {
			Filters filters = objectMapper
					.readValue(filtersJson, Filters.class);
			if(filters==null)return pageBean;
			...
			if(StringUtils.isNotEmpty(rootGroupOp)){
				rootRelateType =  RelateType
						.valueOf(rootGroupOp.toUpperCase());
			}
